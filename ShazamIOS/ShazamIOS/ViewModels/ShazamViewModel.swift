...
class ShazamViewModel: NSObject, ObservableObject {
    ...
    func startListening(){
        ...
        audioSession.requestRecordPermission { granted in
            ...
            inputNode.installTap(onBus: .zero, bufferSize: 1024, format: recordingFormat) {
                (buffer:
                    AVAudioPCMBuffer, when: AVAudioTime
                ) in
            ...
            do{
                ...
                print( "Error al scanear", error.localizedDescription)
            }
            DispatchQueue.main.async {
                self.recording = true
            }
